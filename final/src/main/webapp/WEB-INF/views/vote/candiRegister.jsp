...
<script>
	...
	function check() {
		if ($("#id").val() == "" || $("#job").val() == ""|| $("#gender").val() == "" || $("#content").val() == ""|| $("#phone").val() == "" || $("#parentPhone").val() == ""|| $("#registerDate").val() == "") {
			alert("모든 사항을 입력해주세요.");
			return false;
		} 
		sessionStroage.setItem("id", $("#id").val());
		sessionStroage.setItem("job", $("#job").val());
		sessionStroage.setItem("gender", $("#gender").val());
		sessionStroage.setItem("content", $("#content").val());
	...
	function userSelectResult(list) {
		...
		for (i = 0; i < list.length; i++) {
			$('#searchResult')
				...
				.append(list[i].ho + "호")
				.append('<button type="button" onclick="userInsert(\'' + list[i].id + '\')" id=\'btnSelect\'>선택</button></br>');
		}
	}//userSelectResult
	...
	<div class="container">
		...
		<div>
			...
			<table class="table">
				<tr>
					...
					<td width="300"><input readonly type="text"
						placeholder="후보자 id" name="id" id="id"> <input type="text"
						placeholder="후보자 이름 입력" id="name">
			...
			<div align="right">
				...
				&nbsp;&nbsp;&nbsp;
				<button class="btn btn-outline-secondary" onclick="return check()">등록하기</button>
